#!/usr/bin/env python3
"""
Single-file static site generator (no JS) supporting 5 modes:

  1) regular            : /{city}-{st}/  + /cost/ + /how-to/ + /contact/
  2) cost               : /{city}-{st}/  + /cost/ (+ /cost/{city-st}/) + /contact/   (NO /how-to/)
  3) state              : /{st}/ then /{st}/{city}/ + /contact/                      (NO /cost/ or /how-to/)
  4) subdomain          : each city is its own subdomain + /contact/                  (NO /cost/ or /how-to/)
  5) regular_city_only  : /{city}-{st}/ + /contact/                                  (NO /cost/ or /how-to/)
...
ENV (optional):
  ...
  COPY_VARIANT=1..5                          # reserved; selects copy variant (not used in this file)
"""
...
class SiteConfig:
  ...
  image_filename: str = "pool-mastic-replacement-service.jpg"  # sits next to generate.py

  # Identity
  base_name: str = "Pool Mastic Replacement"
  brand_name: str = "Pool Mastic Replacement"
  ...
  # Core titles/subs
  h1_title: str = "Pool Mastic Replacement/Repair and Pool Caulking Services"
  h1_short: str = "Pool Mastic Replacement Services"
  h1_sub: str = "Flexible joint repairs that protect pool decks, coping, and expansion joints from cracking and water damage."
  ...
  main_h2: tuple[str, ...] = (
    ...
    "Why Does Pool Mastic Crack or Fail Over Time?",
    "What Problems Can Failed Pool Mastic Cause Around a Pool?",
    "How Do You Know When Pool Mastic Needs Replacement?",
    "How Is Pool Mastic Replacement Different From Pool Caulking?",
    "What Happens If Pool Mastic Is Left Unrepaired?",
  )

  main_p: tuple[str, ...] = (
    "Pool mastic is a flexible joint material used between the pool deck and coping to allow movement and keep water out. It’s installed in the expansion joint so the deck and pool can expand and contract without cracking or shifting.",
    "Pool mastic cracks or fails over time because of constant sun exposure, temperature changes, moisture, and normal deck movement. As the material dries out and loses flexibility, it can harden, shrink, or pull away from the joint.",
    "Failed pool mastic can allow water to seep into the expansion joint and surrounding areas of the pool. Over time, this moisture can lead to deck cracking, loose coping, erosion beneath the concrete, and more expensive structural repairs.",
    "You can usually tell pool mastic needs replacement when it becomes brittle, cracked, or separates from the joint edges. Gaps, missing sections, or hardened material are signs the seal is no longer protecting the pool properly.",
    "Pool mastic replacement is different from pool caulking because mastic is designed to handle movement in expansion joints. Standard caulk is more rigid and often breaks down quickly in pool environments, leading to repeat failures.",
    "Leaving pool mastic unrepaired allows water to continue entering areas it shouldn’t, which can worsen damage over time. What starts as a small joint issue can eventually affect the deck, coping, and even the pool shell if ignored."
  )
  ...
  howto_h2: str = "How Woodpecker Damage Is Typically Identified on a House"

  howto_p: str = (
    "Woodpecker damage is typically identified by spotting round holes, clusters of small punctures, or repeated "
    "cavities in siding and trim. These marks often appear on exterior wood surfaces and may vary in depth depending "
    "on how long the bird has been active."
  )
  ...
  location_cost_h2: str = "How Much Does Pool Mastic Replacement Cost{loc}?"

  location_cost_p: str = (
    "Typically pool mastic replacement projects{loc} range from {cost_lo} to {cost_hi}, "
    "depending on the length of the expansion joint, access around the pool, and the condition of the existing material. "
    "Pricing can also vary based on local labor rates and whether old mastic needs to be fully removed."
  )
...
HOWTO_INNER = (
  f"<h2>{esc(CONFIG.howto_h2)}</h2>\n"
  f"<p>{esc(CONFIG.howto_p)}</p>\n"
  "<h2>Typical repair approach</h2>\n"
  "<p>Most repairs involve cleaning and drying the area, filling or patching the void, restoring the surface, and matching paint or finish. "
  "If the area is soft, crumbling, or repeatedly targeted, replacement of trim/siding sections is often the most durable fix.</p>\n"
  "<h2>When to call a pro</h2>\n"
  "<p>If holes are high up, the material is rotted, or multiple areas need finish matching, a professional repair usually saves time and avoids repeat damage.</p>\n"
)

COST_INNER = (
  "<h2>What affects price</h2>\n"
  "<p>Pricing is driven by the number of holes, material type, accessibility (ladder height), and how much finish matching is required.</p>\n"
  "<h2>Common scopes</h2>\n"
  "<p>Small patch and paint is typically the lowest-cost scope. Replacing trim boards, corner boards, or fascia sections is higher cost but often longer lasting.</p>\n"
)
...
<div id="networx_form_container" style="margin:0px;padding:0px;">
    <div id = "nx_form" style = "width: 242px; height: 375px;">
        <script type="text/javascript" src = "https://api.networx.com/iframe.php?aff_id=73601bc3bd5a961a61a973e92e29f169&aff_to_form_id=8024"></script>
    </div>
</div>
""".strip()
